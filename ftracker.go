...
func swimigDistanse(lengthPool, countPool int) float64 {
	swimDistance := (lengthPool * countPool) / mInKm
	return float64(swimDistance)
}
...
func ShowTrainingInfo(action int, trainingType string, duration, weight, height float64, lengthPool, countPool int) string {
	...
	case trainingType == "Плавание":
		swdistance := swimigDistanse(lengthPool, countPool)                        // вызовите здесь необходимую функцию
		...
		return fmt.Sprintf("Тип тренировки: %s\nДлительность: %.2f ч.\nДистанция: %.2f км.\nСкорость: %.2f км/ч\nСожгли калорий: %.2f\n", trainingType, duration, swdistance, speed, calories)
	default:
...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	// ваш код здесь
	resultRun := ((runningCaloriesMeanSpeedMultiplier * meanSpeed(action, duration) * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH)
	return float64(resultRun)
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	resultWalk := ((walkingCaloriesWeightMultiplier*weight + (math.Pow(meanSpeed(action, duration), 2)/height)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
	return float64(resultWalk)
}
